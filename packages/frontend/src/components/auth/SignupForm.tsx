/**
 * Signup form with multi-step Authentik enrollment flow.
 *
 * Renders email, username, password, confirm-password fields.
 * On submit, advances to email verification if required.
 */
...
export function SignupForm({ onSuccess, onLoginClick }: SignupFormProps): JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await submitSignupFlow(flowToken, {
        email: formData.email,
        username: formData.username,
        password: formData.password,
        password_repeat: formData.password,
      });
  ...
  if (step === 'email_verification') {
    return (
      <EmailVerification
        onResend={async () => {
          if (flowToken) await resendVerificationEmail(flowToken);
        }}
